...
@Client.on_message(filters.command("startnow3") & filters.user(AUTH_USERS))
...
@Client.on_message(filters.command("setconfig3") & filters.user(AUTH_USERS))
    ...
    try:
        ...
        if len(parts) != 7:
            await message.reply("Error: Invalid command format. Expected format is:\n"
                                "`/setconfig3 subject_and_channel chat_id courseid bname hour minute`")
            return
        ...
        await message.reply(f"**𝐂𝐨𝐧𝐟𝐢𝐠𝐮𝐫𝐚𝐭𝐢𝐨𝐧🦋🎉🎊vsp3 𝐮𝐩𝐝𝐚𝐭𝐞𝐝 𝐬𝐮𝐜𝐜𝐞𝐬𝐬𝐟𝐮𝐥𝐥𝐲**:\n\n"
...
@Client.on_message(filters.command("viewconfig3") & filters.user(AUTH_USERS))
